...
simple_xml ="<root><rec>one</rec></root>"


@pytest.fixture
def xml_single_test_record():
    import datetime
    from generated import DB_TestXML

    return DB_TestXML(Payload=simple_xml)


def test_xmlInsertReturningSimple(postgres14p2_db):
    from sqlalchemy.orm import Session
    from generated import DB_TestXMLInsertReturning
    import datetime

    session = Session(postgres14p2_db)
    rec = DB_TestXMLInsertReturning.execute(session,
                                            simple_xml)
    session.commit()

    res = postgres14p2_db.execute("SELECT ID, payload "
                                  "FROM ToDoList_App.TestXML "
                                  "WHERE ID=" + str(rec.ID))
